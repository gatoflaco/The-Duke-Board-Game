...
class Player:
    ...
    def setup_phase(self):
        ...
        for troop_name in STARTING_TROOPS:
            ...
            if troop_name == 'Duke':
                self.__duke = self.__in_play[-1]  # the most recently added is the Duke
            i += 1
    ...
    def take_turn(self):
        ...
        Operates based on what is currently in the self.__choices dict, a special dict whose format is documented in
        docs/choice_formats.txt. The dict should contain all the things the player could legally do at the moment. In
        order for this to be true, game.py should maintain the dict for the player, updating it as needed any time the
        board state changes.
        ...
        if len(self.__choices['pull']) != 0 and randrange(0, 10) < 2:
            x, y = self.__choices['pull'][0]
            ...
            troop_locs = list(self.__choices['act'].keys())
            shuffle(troop_locs)
            for troop_loc in troop_locs:
                for teammate_loc in self.__choices['act'][troop_loc]['commands']:
                    if len(self.__choices['act'][troop_loc]['commands'][teammate_loc]) != 0:
                        return {
                            'action_type': 'cmd',
                            'src_location': teammate_loc,
                            'dst_location': self.__choices['act'][troop_loc]['commands'][teammate_loc][
                                randrange(0, len(self.__choices['act'][troop_loc]['commands'][teammate_loc]))],
                            'cmd_location': troop_loc,
                        }
                if len(self.__choices['act'][troop_loc]['strikes']) != 0:
                    return {
                        'action_type': 'str',
                        'src_location': troop_loc,
                        'str_location': self.__choices['act'][troop_loc]['strikes'][
                            randrange(0, len(self.__choices['act'][troop_loc]['strikes']))]
                    }
                if len(self.__choices['act'][troop_loc]['moves']) != 0:
                    return {
                        'action_type': 'mov',
                        'src_location': troop_loc,
                        'dst_location': self.__choices['act'][troop_loc]['moves'][
                            randrange(0, len(self.__choices['act'][troop_loc]['moves']))]
                    }
        if len(self.__choices['pull']):
            x, y = self.__choices['pull'][0]
            tile = self.play_new_troop_tile(x, y)
            return {
                'action_type': 'pull',
    ...
    def remove_from_play(self, x, y, is_captured=True):
        """Removes the troop found at (x, y) from this player's self.__in_play list.
    ...
    def get_tile_with_coords(self, x, y):
        """Searches for the troop in this player's self.__in_play list with coordinates (x, y).
